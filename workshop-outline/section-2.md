...
Get our bearings in the plugin's file layout and the moving parts of an Interactivity API block, so we recognize each piece as we build it up in sections 4–8. We're touring the *finished* slider here — the file we'll write to in each coding section is called out so we know where we're headed.
